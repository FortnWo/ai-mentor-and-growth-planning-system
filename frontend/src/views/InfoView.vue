...
const infoForm = reactive<InfoFormState>({
  full_name: '',
  major: '',
  year_of_study: '',
  bio: '',
...
function syncFormFromInfo(user: UserRead) {
  infoForm.full_name = user.full_name ?? ''
  infoForm.major = user.major ?? ''
  infoForm.year_of_study = user.year_of_study ? String(user.year_of_study) : ''
  infoForm.bio = user.bio ?? ''
}
...
async function saveInfo() {
  ...
  try {
    ...
    const updated = await updateMyInfo({
      ...
      major: infoForm.major.trim() || undefined,
      year_of_study: parseOptionalInteger(infoForm.year_of_study),
      bio: infoForm.bio.trim() || undefined,
    })
...
<template>
  <div class="page page--wide info-page">
    ...
    <div class="grid-2 info-grid">
      <section v-if="userInfo" class="panel info-summary reveal reveal--delay-1">
        <div class="title-row">
          <div>
        ...
        <div class="summary-list">
          <p><strong>用户名</strong><span>{{ userInfo.username }}</span></p>
          <p><strong>邮箱</strong><span>{{ userInfo.email }}</span></p>
          <p><strong>角色</strong><span>{{ userInfo.role }}</span></p>
          <p v-if="userInfo.last_login_at">
            <strong>上次登录</strong><span>{{ new Date(userInfo.last_login_at).toLocaleString() }}</span>
          </p>
        </div>
      </section>

      <form class="panel form-card reveal reveal--delay-2" @submit.prevent="saveInfo">
        <div class="title-row">
          <div>
        ...
        <label class="field">
          ...
          <input v-model="infoForm.full_name" class="input" />
        </label>

        <label class="field">
          <span class="label">专业</span>
          <input v-model="infoForm.major" class="input" />
        </label>

        <label class="field">
          <span class="label">年级</span>
          <input v-model="infoForm.year_of_study" class="input" type="number" min="1" max="12" />
        </label>

        <label class="field span-2">
          <span class="label">简介</span>
          <textarea v-model="infoForm.bio" class="textarea" rows="4"></textarea>
        </label>
