...
def create_student_user(client, index: int = 1):
    response = client.post(
        ...
        json={
            ...
            "email": f"info_{index}@example.com",
            "password": "Student@12345",
            "full_name": f"Info Student {index}",
            "major": "Engineering",
            "year_of_study": 1,
            "bio": "Info testing student",
            "role": "user",
        },
...
def test_info_me_requires_auth(client):
    response = client.get("/info/me")
    assert response.status_code == 401


def test_info_me_read_update_and_password_change(client):
    student = create_student_user(client, 1)
    token = login_student(client, student["username"], "Student@12345")
    headers = {"Authorization": f"Bearer {token}"}

    info_response = client.get("/info/me", headers=headers)
    assert info_response.status_code == 200
    assert info_response.json()["username"] == student["username"]

    update_response = client.put(
        "/info/me",
        json={
            "full_name": "Updated Info Student",
    ...
    assert update_response.json()["full_name"] == "Updated Info Student"
