...
def test_create_action_plan_persists_items(client, monkeypatch):
    ...
    goal_scoped = client.get("/action-plans", params={"goal_id": goal_id}, headers=headers)
    assert goal_scoped.status_code == 200
    assert len(goal_scoped.json()) == 1
    assert goal_scoped.json()[0]["main_breakdown_id"] == root["id"]
